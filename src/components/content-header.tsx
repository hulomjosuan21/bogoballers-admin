...
export default function ContentHeader({ title, breadcrumbs, children }: ContentHeaderProps) {
    return (
                                ...
                                return (
                                    <div key={index} className="flex items-center">
                                        <BreadcrumbItem className={index !== breadcrumbs.length - 1 ? "hidden md:block" : ""}>
                                            {isLast ? <BreadcrumbPage>{crumb}</BreadcrumbPage> : <BreadcrumbLink href="#">{crumb}</BreadcrumbLink>}
                                        </BreadcrumbItem>
