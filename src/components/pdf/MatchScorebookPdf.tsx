...
export default function ScorebookMultiDocument({
  matches,
}: {
  matches: MatchBook[];
}) {
      ...
      {matches.map((match, index) => (
        ...
        >
          <View>
            <Text style={styles.header}>BOGOBALLERS SCORESHEET</Text>
            <Text style={styles.subHeader}>
              Match ID: {match.public_match_id} | Time Remaining:{" "}
              {formatTime(match.time_seconds)} |{" "}
              {getQuarterLabel(match.current_quarter)}
            </Text>
          </View>

          <View style={styles.rowContainer}>
            <View style={styles.halfWidth}>
              <TeamTable team={match.home_team} title="HOME" />
            </View>
            <View style={styles.halfWidth}>
              <TeamTable team={match.away_team} title="AWAY" />
            </View>
          </View>
...
export const printMultipleMatches = async (matches: MatchBook[]) => {
  const blob = await pdf(<ScorebookMultiDocument matches={matches} />).toBlob();
  const url = URL.createObjectURL(blob);
  window.open(url, "_blank");
};
