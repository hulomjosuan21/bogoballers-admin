...
export default function RegisterForm() {
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        ...
        setLoading(true)
        try {
            const res = await LeagueAdministratorService.registerLeagueAdmin(formData)
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div>
                    <ButtonLoading type='submit' className="w-full" loading={isLoading}>Register</ButtonLoading>
                </div>
            </form>
        </Form>
    )
